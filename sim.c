...
void handle_sh(unsigned int cur_inst) {
    ...
    MEMORY[sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1]] = tmp;
    MEMORY[sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1] + 1] = tmp >> 8;
...
void handle_sw(unsigned int cur_inst) {
    ...
    MEMORY[sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1] + 1] = CURRENT_LATCHES.REGS[rs2] >> 8;
    MEMORY[sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1] + 2] = CURRENT_LATCHES.REGS[rs2] >> 16;
    MEMORY[sext(imm12, 12) + CURRENT_LATCHES.REGS[rs1] + 3] = CURRENT_LATCHES.REGS[rs2] >> 24;
